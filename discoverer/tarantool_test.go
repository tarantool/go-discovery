...
func TestNewTarantool_missing(t *testing.T) {
	...
	assert.ErrorIs(t, err, discoverer.ErrMissingStorage)
}

func TestTarantool_Discovery_ctx_deadline(t *testing.T) {
	doer := test_helpers.NewMockDoer(t).AddResponseError(errors.New("any"))
...
func TestTarantool_Discovery_cancelled_context(t *testing.T) {
	doer := test_helpers.NewMockDoer(t).AddResponseError(errors.New("any"))
	client := &mockTntClient{doer: doer}
...
func TestTarantool_Discovery_return_error(t *testing.T) {
	doer := test_helpers.NewMockDoer(t).
		AddResponseError(errors.New("any"))
	client := &mockTntClient{doer: doer}
...
func TestTarantool_Discovery_invalid_data(t *testing.T) {
	doer := test_helpers.NewMockDoer(t).
		AddResponseRaw(makeSingleTcsResponse("- foo\n2"))
	client := &mockTntClient{doer: doer}
...
func TestTarantool_Discovery(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			resp := makeTcsResponse(tc.Values)
			doer := test_helpers.NewMockDoer(t).
				AddResponseRaw(resp).
				AddResponseRaw(resp)
			client := &mockTntClient{doer: doer}
